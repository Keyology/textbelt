...
// Sentry.init({
//   dsn: "https://1a07780f3c74402fadff8bdad7480576@sentry.io/1525166",
...
function textRequestHandler(req, res, number, carrier, region) {
  ...
  text.send(number, message, carrierKey, region, err => {
    if (err) {
      res.send({
        success: false,
        message: `Communication with SMS gateway failed. Did you configure mail transport in lib/config.js?  Error message: '${
          err.message
        }'`
      });
    } else {
